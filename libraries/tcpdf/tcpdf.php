...
class TCPDF {
	...
	public function __construct($orientation='P', $unit='mm', $format='A4', $unicode=true, $encoding='UTF-8', $diskcache=false) {
		...
		if (file_exists(__DIR__ . '/spotcolors.php')) {
			require(__DIR__ . '/spotcolors.php');
		...
		if ($txt != '') {
			...
			if (($align == 'J') AND ($ns > 0)) {
				if ($this->isUnicodeFont()) {
					...
					$spacewidth = -1000 * ($w - $width - $this->cell_padding['L'] - $this->cell_padding['R']) / ($ns?:1) / $this->FontSize;
					if ($this->font_stretching != 100) {
	...
	protected function _putviewerpreferences() {
		...
		if (isset($this->viewer_preferences['NumCopies'])) {
			$out .= ' /NumCopies ' . (int)$this->viewer_preferences['NumCopies'];
		}
		$out .= ' >>';
		return $out;
	}
					...
					if ((isset($plalign) AND ((($plalign == 'C') OR ($plalign == 'J') OR (($plalign == 'R') AND (!$this->rtl)) OR (($plalign == 'L') AND ($this->rtl)))))) {
						...
						if ($this->isRTLTextDir()) { // RTL
							...
							if ($pos1 > 0) {
								$pos1 = (int)$pos1;
								if ($this->isUnicodeFont()) {
									$pos2 = (int)$this->revstrpos($pmid, '[(' . chr(0) . chr(32));
									$spacelen = 2;
								} else {
									$pos2 = (int)$this->revstrpos($pmid, '[(' . chr(32));
									$spacelen = 1;
								}
								if ($pos1 == $pos2) {
							...
							if ($pos1 > 0) {
								$pos1 = (int)$pos1;
								if ($this->isUnicodeFont()) {
									$pos2 = (int)$this->revstrpos($pmid, chr(0) . chr(32) . ')]') + 2;
									$spacelen = 2;
								} else {
									$pos2 = (int)$this->revstrpos($pmid, chr(32) . ')]') + 1;
									$spacelen = 1;
								}
								if ($pos1 == $pos2) {
